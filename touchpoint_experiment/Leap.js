...
Leap.PointableList.prototype.leftmost = function() {
  ...
  for (i = 0; i < this.length; i++) {
    if ((leftmost._valid == false || this[i].tipPosition().x < leftmost.tipPosition().x) && this[i]._valid) leftmost = this[i];
  }
...
Leap.PointableList.prototype.rightmost = function() {
  ...
  for (i = 0; i < this.length; i++) {
    if ((rightmost._valid == false || this[i].tipPosition().x > rightmost.tipPosition().x) && this[i]._valid) rightmost = this[i];
  }
...
Leap.PointableList.prototype.frontmost = function() {
  ...
  for (i = 0; i < this.length; i++) {
    if ((frontmost._valid == false || this[i].tipPosition().z < frontmost.tipPosition().z) && this[i]._valid) frontmost = this[i];
  }
...
Leap.Calibrate = function(controller) {
  ...
  var me = this;
  this._listener = new Leap.Listener();
  this._listener.onFrame = function(controller) {
    me._fingerCount(controller);
  };
...
Leap.Calibrate.prototype = {
  _pointCSS:
    "width: 20px; height: 20px; padding: 10px; margin: -20px; position: fixed; text-align: center; background-color: #c3cccc; color: #ffffff; cursor: pointer; ",
  _point1CSS: "left: 25%; top: 50%;",
  _point2CSS: "left: 25%; top: 25%;",
  _point3CSS: "left: 75%; top: 50%;",

  _tipCSS:
    "height: 32px; padding-top: 4px; padding-bottom: 4px; padding-left: 8px; padding-right: 8px; margin-top: -20px; margin-left: 26px; border-radius: 5px; position: fixed; background-color: rgba(0,0,0,0.7); color: #FFFFFF; font: 14px Helvetica, Sans-Serif;",
  _arrowCSS:
    "height: 0; width: 0; border: 6px solid; border-color: transparent rgba(0,0,0,0.7) transparent transparent; margin-top: -6px; margin-left: 14px; position: fixed;",
  ...
  _fingerCount: function(controller) {
    ...
    for (var i = 0; i < count; i++) {
      ...
      if (pointable.direction().angleTo(this._deviceNormal) < Math.PI / 8) {
        if (this._goodFinger == null || this._goodFinger.tipPosition().z > pointable.tipPosition().z) this._goodFinger = pointable;
      }
    }
...
Leap.Controller.prototype = {
  ...
  _onmessage: function(event) {
    ...
    for (index in this._listeners) this._listeners[index].onFrame(this);
  },
  ...
  _connect: function(connection) {
    ...
    this._socket.onopen = function(event) {
      ...
      for (index in this._controller._listeners) this._controller._listeners[index].onConnect(this._controller);
    };

    this._socket.onclose = function(event) {
      console.log("Socket Closed");
      this.connected = false;
      for (index in this._controller._listeners) this._controller._listeners[index].onDisconnect(this._controller);
      var me = this;
      setTimeout(function() {
        me._controller._connect(me.url);
      }, 1000);
    };
...
Leap.CircleGesture.prototype.normal = function() {
  return this._normal;
};
Leap.CircleGesture.prototype.pointable = function() {
  return this._pointable;
};
Leap.CircleGesture.prototype.progress = function() {
  return this._progress;
};
Leap.CircleGesture.prototype.radius = function() {
  return this._radius;
};
Leap.CircleGesture.prototype.center = function() {
  return this._center;
};
Leap.CircleGesture.prototype.isClockwise = function() {
...
Leap.KeyTapGesture.prototype.pointable = function() {
  return this._pointable;
};
Leap.KeyTapGesture.prototype.position = function() {
  return this._position;
};
Leap.KeyTapGesture.prototype.progress = function() {
  return this._progress;
};
...
Leap.ScreenTapGesture.prototype.pointable = function() {
  return this._pointable;
};
Leap.ScreenTapGesture.prototype.position = function() {
  return this._position;
};
Leap.ScreenTapGesture.prototype.progress = function() {
  return this._progress;
};
...
Leap.SwipeGesture.prototype.direction = function() {
  return this._direction;
};
Leap.SwipeGesture.prototype.pointable = function() {
  return this._pointable;
};
Leap.SwipeGesture.prototype.position = function() {
  return this._position;
};
Leap.SwipeGesture.prototype.speed = function() {
  return this._speed;
};
Leap.SwipeGesture.prototype.startPosition = function() {
...
Leap.Matrix.prototype = {
  ...
  compare: function(other) {
    return (
      this.xBasis.compare(other.xBasis) && this.yBasis.compare(other.yBasis) && this.zBasis.compare(other.zBasis) && this.origin.compare(other.origin)
    );
  }
};

Leap.Matrix.identity = function() {
  return new Leap.Matrix();
};
...
Leap.Screen.invalid = function() {
  return new Leap.Screen();
};
...
Leap.Vector.backward = function() {
  return new Leap.Vector([0, 0, 1]);
};
Leap.Vector.down = function() {
  return new Leap.Vector([0, -1, 0]);
};
Leap.Vector.forward = function() {
  return new Leap.Vector([0, 0, -1]);
};
Leap.Vector.left = function() {
  return new Leap.Vector([-1, 0, 0]);
};
Leap.Vector.right = function() {
  return new Leap.Vector([1, 0, 0]);
};
Leap.Vector.up = function() {
  return new Leap.Vector([0, 1, 0]);
};
Leap.Vector.xAxis = function() {
  return new Leap.Vector([1, 0, 0]);
};
Leap.Vector.yAxis = function() {
  return new Leap.Vector([0, 1, 0]);
};
Leap.Vector.zAxis = function() {
  return new Leap.Vector([0, 0, 1]);
};
Leap.Vector.zero = function() {
  return new Leap.Vector([0, 0, 0]);
};
